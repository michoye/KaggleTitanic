...
-   Add other models that are of interest - e.g. xgboost, lightgbm, and/or nnet
...
```{r}
# Create the workflow with the glm model
glm_models = 
...
# Grid creation
basic_grid <- expand_grid(penalty_vals, mix_vals)
spline_grid <- expand_grid(penalty_vals, mix_vals, age_vals, fare_vals)
grid_list <- list(
  basic_glm = basic_grid,
...
# Examine the results
tune_results_list %>%
  extract_workflow_set_result("splines_glm") %>% # or other workflow identifier
