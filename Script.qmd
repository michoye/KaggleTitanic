...
-   Think about imputation / try first imputation
    ...
    -   **TODO impute fare and embarkment**
        -   impute the two missing values as southampton
        -   remove zeroes from fare and impute with mean from Pclass
    -   TODO add columns for children who travelled with mom / mom traveling with children
    -   TODO We need an indicator for the family unit
...
### Adding family indicators

-   We want an indicator for a mom travelling with children

-   And an indicator for children travelling with a mom

Note:

Not sure whether we should add these variables this way. Added value comes from family unit (Mother + child units) as these often Survived or Died together. Just adding the columns 'Mother(OnDuty)' and/or 'Children(InCare)' doesn't necessarily add value and can quite easily be derived by the existing columns age and/or gender. So either go for the variable family unit and create many different categories or leave them out.
...
data_imput |> 
    ...
    filter(Parch >= 1) |> 
    group_by(Ticket) |> 
    filter(n() >= 2) |>  # Ensure at least 2 instances per Ticket group
    ungroup() |> 
    filter(Sex == "female") |>  # Keep only females
    group_by(Ticket) |> 
    slice_max(Age, n = 1, with_ties = FALSE) |>  # Oldest female in each Ticket group
    ungroup() |> 
    filter(title == " Mrs.") |>  # Filter by title
    View()

# New column created
data_imput <- data_imput |> 
    dplyr::mutate(
        MotherOnDuty = if_else(
            Parch >= 1 & 
            Sex == "female" & 
            title == " Mrs." & 
            Ticket %in% (
...
data_imput |> 
    ...
    filter(Parch >= 1, title == " Miss." | title == " Master.") |> 
  view()
```
