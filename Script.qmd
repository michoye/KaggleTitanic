...
-   xgboost: find error related the id of the workflow
...
models_set |> 
  dplyr::filter(
    wflow_id == "basic_xgboost"
  )
```

## Workflow map to store hypertuning parameters
...
tune_results_list <- 
  ...
  option_add(grid = spline_grid, id = "splines_glm") |> 
  option_add(grid = spline_net_grid, id = "splines_glm_net") %>%
  workflow_map(
    "tune_grid",
    resamples = folds,
...
```{r}
tune_results_list |> 
  extract_workflow_set_result(id = "basic_glm") |> 
