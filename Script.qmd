...
analysis_data2 = 
  ...
  dplyr::mutate(
    across(
      c(Survived, Pclass, Sex, SibSp, Parch, Embarked, Cabin_sector, title_group),
      as.factor),
    across(
      c(Age, Fare),
      round
...
penalty_grid <- grid_regular(penalty(range = c(-4, -1)), levels = 20)  # e.g. 20 lambda values from 10^-4 to 10^-1

mix_vals <- tibble(mixture = c(0, 0.25, 0.5, 0.75, 1.0))

grid_vals <- expand_grid(penalty_grid, mix_vals)
```

```{r}
# Set up a workflow with a recipe (if needed for preprocessing)
wf <- 
  workflow() |> 
  add_model(glm_net_mod) |> 
  add_recipe(
    )
```
